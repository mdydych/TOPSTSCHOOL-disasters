...
if (httr::status_code(res) == 200) {
  base::message("Download successful")
} else {
  base::stop("Failed to download file")
}
...
# Note: Data is already in UTM Zone 11N (EPSG:26911)
...
ggplot2::ggplot() +
  ...
  ggplot2::labs(
    ...
    subtitle = paste0("Municipal Area: ", 
                     base::round(sf::st_area(yk_boundary)/1000000, 2), 
                     " sq km"),
...
```{r}
  ...
  # Previous code remains the same until the temporal plots
...
stats_table <- knitr::kable(stats_df, 
                           format = "html",
...
Could not get animation working inside the quarto. Check scratch/m302-scratch.R for working code.
...
fire_perimeters <-
  base::do.call(rbind,
                base::lapply(unique_dates, function(d)
...
all_clustered_perimeters <- base::do.call(rbind,
  base::lapply(unique_dates, function(d) {
    perims <- create_clustered_perimeters(firms_points, d)
    if (!base::is.null(perims)) {
      perims$date <- d
    }
    return(perims)
  })
...
# Create progression plot with layered hulls and proper date handling
p4_progression <- ggplot2::ggplot() +
  ...
  ggplot2::geom_sf(
    data = all_clustered_perimeters |>
      dplyr::arrange(dplyr::desc(date)),  # Reverse chronological order
    ggplot2::aes(fill = as.Date(date)),  # Convert to Date explicitly
    alpha = 0.4,
    linewidth = 0.8
  ) +
  ggplot2::scale_fill_viridis_c(
    ...
    breaks = scales::breaks_width("2 days")  # Show date every 2 days
  ) +
  ggplot2::theme_minimal() +
  ggplot2::labs(
    title = "Fire Complex Progression",
    subtitle = base::paste(format(base::min(unique_dates), "%B %d"), 
                          "to", 
                          format(base::max(unique_dates), "%B %d")),
    caption = "Note: Clustered using DBSCAN algorithm (eps = 3km)\nEarlier dates displayed on top"
  )

# Display progression plot
base::print(p4_progression)
...
create_clustered_perimeters <- function(points, date, eps = 3000) {
  # eps in meters
  ...
  hulls <- daily_points |>
    ...
    dplyr::group_map(function(cluster_points, cluster_id) {
      ...
      current_area <- base::as.numeric(units::set_units(sf::st_area(current_poly), "km^2"))
...
p4c <- ggplot2::ggplot() +
  ...
  # Style
  ggplot2::scale_color_viridis_c(name = "Fire Radiative\nPower (MW)", option = "inferno") +
  ggplot2::theme_minimal() +
  ggplot2::labs(title = "Comparison of Fire Complexes", caption = "Note: Points clustered using DBSCAN algorithm (eps = 5km)")
...
for (date_type in base::unique(comparison_clusters$type)) {
  ...
  base::cat("\n", date_type, ":\n")
  base::cat("Number of distinct fire complexes:",
            base::length(base::unique(clusters_date$cluster)),
            "\n")
  base::cat("Total area across all complexes:",
            base::round(base::sum(clusters_date$area_km2), 2),
            "km²\n")
  base::cat("Largest single complex:",
            base::round(base::max(clusters_date$area_km2), 2),
            "km²\n")
}
```
...
- Access GOES-18 (West) for local Yellowknife analysis
  - Better viewing angle for source region
  - Higher accuracy for initial plume development
  - Coverage of western spread

- Access GOES-16 (East) for plume tracking
  - Coverage of eastern movement
  - Tracking of impacts into US Midwest
  - Overlap comparison where available

#### Analysis Components
1. Local Impact Analysis (GOES-18)
   - AOD values around Yellowknife
   - Temporal patterns during evacuation period
   - Relationship to FIRMS detections

2. Plume Progression Analysis (both satellites)
   - Initial plume development (GOES-18)
   - Tracking eastward movement (GOES-16)
   - Extent of smoke coverage
   - Population centers affected

#### Visualization Approach
1. Single-satellite views
   - Local Yellowknife analysis
   - Regional progression maps

2. Combined satellite products
   - Merged view where coverage overlaps
   - Full extent of smoke impact
   - Time series animations

Would you like me to:
1. Start implementing any specific section?
2. Add more detail to the outline?
3. Focus on a particular analysis component?
4. Begin with the visualization code?
